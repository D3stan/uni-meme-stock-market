...
Un'applicazione web gestionale che simula una borsa valori all'interno dell'ateneo. Non esistono i classici "Like" o reazioni social, ma solo **Ordini di Acquisto** e **Ordini di Vendita**.
...
* **Frontend:** HTML5, CSS (**Bootstrap 5**), **Javascript "Vanilla"** (senza framework come React/Vue/Angular), Chart.js per i grafici.
...
* **Bonus Matricola:** Il sistema prevede un meccanismo di incentivazione all'ingresso. Ogni nuovo utente riceve una dotazione iniziale di **100 CFU** (gestita nel controller di registrazione di Laravel).
...
* **Sospensione Titoli (Update):** L'Admin può bloccare le contrattazioni su un titolo (flag `is_halted`) in caso di anomalie di mercato.
...
* **Atomicità:** Grazie a `DB::transaction()` di Laravel, il prelievo dei CFU dal saldo utente, l'accredito delle fee al sistema e l'assegnazione delle azioni avvengono in un unico blocco indivisibile. Se una qualsiasi di queste operazioni fallisce, tutto viene annullato (rollback), garantendo che non si perdano soldi o azioni nel nulla.
...
* **Header Sticky:** Logo + Saldo CFU + Icona Menu.
...
| **`memes`** | `id`, `creator_id` (FK), `category_id`, `title`, `image_path`, `current_price`, `total_shares`, `status`, `created_at`. |
| **`price_histories`** | `id`, `meme_id` (FK), `price`, `recorded_at` (usato per disegnare i grafici). |

### 3. Finanza & Transazioni
| Tabella | Campi Chiave |
| :--- | :--- |
| **`portfolios`** | `id`, `user_id`, `meme_id`, `quantity`, `avg_buy_price` (essenziale per calcolare il P&L %). |
| **`transactions`** | `id`, `user_id`, `meme_id`, `type` (buy, sell, ipo_fee, bonus), `quantity`, `price_per_share`, `fee_amount`, `total_amount`, `executed_at`. |

### 4. Utility
| Tabella | Campi Chiave |
| :--- | :--- |
| **`global_settings`** | `key`, `value`. (Es. `listing_fee`, `tax_rate`). |
| **`watchlists`** | `id`, `user_id`, `meme_id`. |
