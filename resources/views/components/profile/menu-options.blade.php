...
<div class="px-4 mb-6">
    ...
    <div class="space-y-3">
        ...
        {{-- Admin Panel --}}
        @if($isAdmin)
        <a href="{{ route('admin.admin') }}" class="bg-surface-100 rounded-2xl p-5 border border-surface-200 hover:border-surface-200/80 transition-colors flex items-center justify-between group">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-surface-200 rounded-full flex items-center justify-center group-hover:bg-surface-200/80 transition-colors">
                    <span class="material-icons text-text-main text-xl">shield</span>
                </div>
                <span class="text-text-main font-medium">Pannello Admin</span>
            </div>
            <span class="material-icons text-text-muted group-hover:text-text-main transition-colors">chevron_right</span>
        </a>
        @endif
